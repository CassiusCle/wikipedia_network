...
class BatchChunk:
    ...
    def __post_init__(self) -> None:
        ...
        self.logger.info(
            "Chunk {self.chunk_number} of batch {self.batch_number} initialised"
        )
    ...
    def finish_chunk(self) -> bool:
        if self.is_finished:
            self.logger.warning(
                "Chunk {self.chunk_number} of batch {self.batch_number} is already finished"
            )
            return False
        if self.time_start is None:
            self.logger.error(
                "Chunk {self.chunk_number} of batch {self.batch_number} has not been started"
            )
            return False
        ...
        self.logger.info(
            "Chunk {self.chunk_number} of batch {self.batch_number} finished"
        )
        return True
